...
```bash
podman search registry.access.redhat.com/ubi8/ubi
podman search registry.access.redhat.com/ubi8/ubi
```
...
```bash
podman run --name mselif-ubi8 -it registry.access.redhat.com/ubi8/ubi /bin/bash
```
...
```bash
podman start --attach --interactive mselif-ubi8
podman diff mselif-ubi8 
podman start --attach --interactive mselif-ubi8
```
...
```bash
podman run --name mselif-ubi8-httpd -it registry.access.redhat.com/ubi8/ubi /bin/bash
dnf update -y & dnf install -y httpd & dnf clean all
rm -rf /var/cache/dnf
sed -i 's/Listen 80/Listen 8080/g' /etc/httpd/conf/httpd.conf
podman diff mselif-ubi8-httpd
podman commit -a "MS Elif" mselif-ubi8-httpd mselif/mselif-ubi8-httpd
```
